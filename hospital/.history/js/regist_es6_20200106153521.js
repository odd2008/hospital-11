...
var app = new Vue({
	...
	function touchstartHandler(e) {
      ...
      function setTransition(a, b, c) {
        if (a) {
            carouselLis[left].style.transition = 'transform 2.5s';
        } else {
            carouselLis[left].style.transition = 'none';
        }
        if (b) {
            carouselLis[center].style.transition = 'transform 2.5s';
        } else {
            carouselLis[center].style.transition = 'none';
        }
        if (c) {
            carouselLis[right].style.transition = 'transform 2.5s';
        } else {
      ...
      function setTransform(dx) {
        dx = dx || 0;
        $('#ul-imgs li')[left].style.transform = 'translateX(' + (-100) + '%)';
        $('#ul-imgs li')[center].style.transform = 'translateX(' + dx + '%)';
        $('#ul-imgs li')[right].style.transform = 'translateX(' + (100) + '%)';
      }
            // 动态设置小圆点的active类
