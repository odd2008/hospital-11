...
var app = new Vue({
    ...
    methods: {
        ...
        lunbo: function lunbo() {
            ...
            // 初始三个固定的位置
            var carouselUl=ducument.getElementById('ul-imgs');
	...
	function touchstartHandler(e) {
      ...
      function setTransition(a, b, c) {
        if (a) {
          $('#ul-imgs li')[left].style.transition = 'transform 1s';
        } else {
          $('#ul-imgs li')[left].style.transition = 'none';
        }
        if (b) {
          $('#ul-imgs li')[center].style.transition = 'transform 1s';
        } else {
          $('#ul-imgs li')[center].style.transition = 'none';
        }
        if (c) {
          $('#ul-imgs li')[right].style.transition = 'transform 1s';
        } else {
          $('#ul-imgs li')[right].style.transition = 'none';
        }
      }
      ...
      function setTransform(dx) {
        dx = dx || 0;
        $('#ul-imgs li')[left].style.transform = 'translateX(' + (-screenWidth + dx) + '%)';
        $('#ul-imgs li')[center].style.transform = 'translateX(' + dx + '%)';
        $('#ul-imgs li')[right].style.transform = 'translateX(' + (screenWidth + dx) + '%)';
      }
      // 动态设置小圆点的active类
        $
    //   var pointsLis = points.querySelectorAll('li');
      ...
      function setPoint() {
        for (var i = 0; i < $('#ul-imgs li').length; i++) {
            $('#ul-imgs li')[i].removeClass('active');
        }
